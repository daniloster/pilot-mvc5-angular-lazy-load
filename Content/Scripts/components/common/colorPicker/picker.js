            ...
            app.lazy.directive("colorPicker", ['ConfigApp', '$compile', function (ConfigApp, $compile) {
                ...
                return {
                    ...
                    link: function ($scope, elem, attrs, ngModel) {
                        ...
                        }).colpickSetColor(!!ngModel.$modelValue ? ngModel.$modelValue.replace('#', '') : 'transparent');

                        $scope.$watch('colorValue', function (nval, oval) {
                            if (nval === oval) return;
                            picker.colpickSetColor(!!$scope.colorValue && $scope.colorValue != '' ? $scope.colorValue.replace('#', '') : 'transparent');
                        });
