    ...
    define(['angular', 'app', 'components/common/loading/loadingController', 'auth/authorizationController', 'components/common/navMenu/navCheck'], function (angular, app) {
        if (!loaded) {
            app.lazy.directive("menu", ['ConfigApp', function (ConfigApp) {
                angular.element('body').after(angular.element(ConfigApp.getElementLink('/Content/App/components/app/menu/style.css')));
                return {
                    replace: true,
                    restrict: 'EA',
                    templateUrl: ConfigApp.getPath('/Content/App/components/app/menu/template.html')
                };
            }]);
            loaded = true;
        }
